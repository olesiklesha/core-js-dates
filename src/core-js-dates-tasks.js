...
function getNextFriday(date) {
  ...
  if (currDay === 4) nextDate += 7;
  if (currDay < 4) nextDate = 5 - currDay;
  if (currDay > 4) nextDate = 7 - currDay + 5;

  return new Date(Date.UTC(year, month, currDate + nextDate));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (
    Math.ceil((Date.parse(dateEnd) - Date.parse(dateStart)) / SEC_PER_DAY) + 1
  );
}
...
function formatDate(date) {
  ...
  return `${dateObj.getUTCMonth() + 1}/${dateObj.getUTCDate()}/${dateObj.getUTCFullYear()}, ${hours > 12 ? hours - 12 : hours}:${minutes}:${seconds} ${MDPos}`;
}
...
function getNextFridayThe13th(date) {
  ...
  if (day === 13 && currMonth === 11) {
    year += 1;
    currMonth = 0;
  }

  for (let i = currMonth; i < 12; i += 1) {
    const currDate = new Date(year, i, 13);
    if (currDate.getDay() === 5) return currDate;
  }

  return getNextFridayThe13th(new Date(year + 1, 0, 1));
}
